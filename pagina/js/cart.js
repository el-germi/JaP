...
let usuario = "25801";
let URL = CART_INFO_URL + usuario + EXT_TYPE;
...
fetch(URL)
  ...
  .then((res) => {
    ...
    productos.forEach((e) => {
      if (!prodLocal.find((p) => p.id == e.id)) prodLocal.push(e);
    });
  ...
  for (let i = 0; i < cartInfo.length; i++) {
    ...
    productos.innerHTML += `
            <div class="cart-grid" id="div${i}">
                ...
                <div>
                    <input type="number" id="number${i}" value="${
      item.count
    }" min="1" oninput="updateVal(${i})" class="form-control form-control-sm">
                </div>
                <div>
                    <div class="subtotal"><strong>${
                      item.currency
                    } </strong><p id="subtotal${i}">${
...
function actualizarTotal() {
  let fee = 0;
  if (shipping.children[shipping.selectedIndex] == opt15) fee = 0.15;
  if (shipping.children[shipping.selectedIndex] == opt7) fee = 0.07;
  if (shipping.children[shipping.selectedIndex] == opt5) fee = 0.05;
  ...
  cartInfo.forEach((e) => {
    if (e.currency == "UYU") totalUyu += e.count * e.unitCost;
    //if (e.currency == "USD")
    else totalUsd += e.count * e.unitCost;
  });

  if (pesosSwitch.checked)
    //true = USD
    subtotal.innerHTML = totalUsd + uyuToUsd * totalUyu;
  else subtotal.innerHTML = totalUyu + usdToUyu * totalUsd;

  subtotal.innerHTML = Number(subtotal.innerHTML).toFixed(
    pesosSwitch.checked ? 2 : 0
  );
  envio.innerHTML = (subtotal.innerHTML * fee).toFixed(
    pesosSwitch.checked ? 2 : 0
  );
  total.innerHTML = (
    Number(subtotal.innerHTML) + Number(envio.innerHTML)
  ).toFixed(pesosSwitch.checked ? 2 : 0);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  convert("UYU", "USD", 1).then((s) => {
    uyuToUsd = s.result;
    if (pesosSwitch.checked) actualizarTotal();
  });
...
function convert(from, to, amount) {
  return fetch(
    `https://currency-exchange.p.rapidapi.com/exchange?to=${to}&from=${from}&q=${amount}`,
    {
      method: "GET",
      redirect: "follow",
      headers: {
...
function validaciones() {
  ...
  if (!tarjeta.checked && !transferencia.checked) {
    // si no se seleccionó ninguna forma de pago...
    resultado = false;
    document
      .getElementById("formaPago")
      .classList.replace("btn-secondary", "btn-outline-danger");
    document.getElementById("feedback-formaPago").style.display = "inline";
  } else {
    document
      .getElementById("formaPago")
      .classList.replace("btn-outline-danger", "btn-secondary");
